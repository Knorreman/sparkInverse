...
object block {
  implicit class BlockMatrixSyntax(private val matrix: BlockMatrix) extends AnyVal {
    ...
    def hyperpowerInverse(): BlockMatrix = MatrixInversion.block(matrix).hyperpowerInverse()
    def hyperpowerInverse(config: IterativeInverseConfig): BlockMatrix = MatrixInversion.block(matrix).hyperpowerInverse(config)
    def hyperpowerInverse(order: Int): BlockMatrix = MatrixInversion.block(matrix).hyperpowerInverse(order)
    def hyperpowerInverse(order: Int, config: IterativeInverseConfig): BlockMatrix = MatrixInversion.block(matrix).hyperpowerInverse(order, config)
